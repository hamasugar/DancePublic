...
import Alamofire
import SwiftyJSON
//ストーリーボード経由の遷移もextensionで共通化させたいよね　
class TeacherRegisterViewController: UIViewController {
    ...
    @objc func register() {
        ...
        if passwordTextField.text != passwordTextField2.text {
            ...
            let endURL = awsURL + "/teacher/create"
            ...
            Alamofire.request(endURL, method: .post, parameters: parameters, encoding: JSONEncoding.default, headers: headers).responseJSON { response in
                ...
                print (response)
                // lambdaで自分で書いたメッセージはresponse.errorには入らない
    ...
    @objc func goLogin() {
        let mainStoryboard: UIStoryboard = UIStoryboard(name: "Main", bundle: nil)
        let nc = mainStoryboard.instantiateViewController(withIdentifier: "teacherLogin")
